    ...
    def run(self):
        try:
            ...
            while reader.isRunning() and self._running:
                ...
                time.sleep(0.005)  # Add 100ms delay per iteration
        ...
        try:
            if self.task_type == "direct_run":
                ...
                try:
                    ...
                    self.output_signal.emit(f"error can not import paste block: {str(e)}")
...
class SegmentCanvas(FigureCanvas):
    ...
    def plot_single_recognition_result(self, result):
        ...
        # Handle empty or invalid probs
        if not isinstance(probs_dict, dict) or not probs_dict:
            self.axes.set_title("Invalid probabilities")
            self.draw()
            return

        # Extract class names and probabilities
        class_labels = sorted(probs_dict.keys())  # Sort for consistent order
        probs = [probs_dict[cls] for cls in class_labels]
        num_classes = len(class_labels)

        # Find the index of the recognized class
        recognized_idx = class_labels.index(label) if label in class_labels else -1

        # Create bar chart
        indices = np.arange(num_classes)
        bar_width = 0.4
        colors = ['blue'] * num_classes  # Default color for all bars
        if recognized_idx >= 0:
            colors[recognized_idx] = 'red'  # Highlight recognized class in red

        bars = self.axes.bar(indices, probs, bar_width, color=colors, alpha=0.7)
        ...
        self.axes.set_title(f"Recognition Result: {filename}")
        self.axes.set_xlabel("Class")
        self.axes.set_ylabel("Probability")
        self.axes.set_xticks(indices)
        self.axes.set_xticklabels(class_labels, rotation=0, ha='right')

        # Add text for recognized label and comm_code
        self.axes.text(
            0.5, 0.1,
            f"Recognized: {label} ({comm_code})",
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.video_label = QLabel("video play aera")
        ...
        self.segment_display_label = QLabel("segmentation result:")
        ...
        self.show_all_btn = QPushButton("show all frame result")
        ...
        self.load_btn = QPushButton("load AEDAT4 file")
        self.output_dir_btn = QPushButton("Select Output Directory")
        self.checkpoint_btn = QPushButton("Select Model")  # New button
        self.file_label = QLabel("Current file：none")
        self.output_dir_label = QLabel("Output Directory: None")
        self.checkpoint_label = QLabel("Select Model Directory")  # New label for checkpoint
        ...
        # Processing buttons
        self.segment_btn = QPushButton("Perform Segmentation")
        self.recognize_btn = QPushButton("Perform Recognition")
        self.single_recognize_btn = QPushButton("Recognize Single File")

        # New: List widget for segmented files
        self.segmented_files_list = QListWidget()
        self.segmented_files_list.setMaximumHeight(100)
        self.segmented_files_list.currentRowChanged.connect(self.update_selected_file)
        ...
        left_layout.addWidget(self.single_recognize_btn)
        left_layout.addWidget(QLabel("Segmented Files List:"))
        left_layout.addWidget(self.segmented_files_list)
        left_layout.addStretch()

        # Right results panel
        right_layout = QVBoxLayout()
        self.result_label = QLabel("Recognition Results:")
        ...
        self.single_recognize_btn.clicked.connect(self.run_single_recognition)
    ...
    def load_file(self):
        ...
        path, _ = file_dialog.getOpenFileName(self, "select AEDAT4 file", default_dir, "AEDAT4 Files (*.aedat4)")
        if path:
            self.file_path = path
    ...
    def select_output_dir(self):
        ...
        if dir_path:
            ...
            self.console_output.append(f"Output directory selected:{dir_path}")
    ...
    def update_button_states(self):
        ...
        # self.recognize_btn.setEnabled(False)
        self.recognize_btn.setEnabled(bool(self.segmented_files and self.checkpoint_path))
        self.single_recognize_btn.setEnabled(bool(self.selected_segmented_file and self.checkpoint_path))
    ...
    def stop_video(self):
        if self.video_thread and self.video_thread.isRunning():
            ...
            self.video_label.setText("Video Playback Area")
            self.video_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def run_segmentation(self):
        if not self.file_path or not self.output_dir:
            self.console_output.append("Please select the AEDAT4 file and output directory first")
            return
        if not os.path.isfile(self.file_path):
            self.console_output.append(f"Error file {self.file_path} does not exist")
            return
        if not os.path.isdir(self.output_dir) or not os.access(self.output_dir, os.W_OK):
            self.console_output.append(f"Error file Output directory {self.output_dir} does not exist")
            return

        self.console_output.clear()
        self.segment_list.clear()
        self.segmented_files_list.clear()
        self.segmented_files = []
        self.console_output.append(f"Start processing the file: {self.file_path}")
        self.console_output.append(f"Output Directory: {self.output_dir}")
    ...
    def select_checkpoint(self):
        ...
        if path:
            ...
            self.checkpoint_label.setText(f"checkpoint:{os.path.basename(path)}")
            self.console_output.append(f"Selected checkpoint: {path}")
            self.recognizer = None  # Reset recognizer to force reinitialization
            self.recognize_btn.setEnabled(bool(self.file_path and self.output_dir and self.checkpoint_path))
        else:
            self.console_output.append("No checkpoint file selected")
    ...
    def run_single_recognition(self):
        if not self.selected_segmented_file:
            self.console_output.append("Error Please select a .npz file first")
            return
        if not self.checkpoint_path:
            self.console_output.append("Error Please select a model checkpoint first")
            return

        self.console_output.append(f"Start categorizing the selected files: {os.path.basename(self.selected_segmented_file)}")
        try:
            ...
            results, summary = self.recognizer.classify_npz_files([self.selected_segmented_file])
            ...
            for i, result in enumerate(results):
                ...
                self.console_output.append(item_text)

            self.console_output.append(summary)

            if results:
                self.recognition_results = results
                self.result_list.setCurrentRow(0)  # Trigger visualization of the first result
        except Exception as e:
            self.console_output.append(f"ERROR An error occurred during classification: {str(e)}")
            import traceback
            self.console_output.append(traceback.format_exc())

    def run_recognition(self):
        if not self.output_dir:
            self.console_output.append("Error Please select the output directory first")
            return
        ...
        if not hasattr(self, 'npz_files') or not self.npz_files:
            self.console_output.append("ERROR No .npz file available")
            return

        self.console_output.append(f"found {len(self.npz_files)} .npz files, starting classification...")
        # Find .npz files in output_dir
        npz_file_paths = glob.glob(os.path.join(self.output_dir, "*.npz"))
        if not npz_file_paths:
            self.console_output.append("ERROR No .npz file found in output directory")
            return

       #  self.console_output.append(f"找到 {len(npz_file_paths)} 个 .npz 文件，开始分类...")
        self.console_output.append(f"Start categorizing the selected files: {self.npz_files}")
        try:
    ...
    def process_output(self, line):
        ...
        if line.startswith("Recognition"):
            result = line.replace("Recognition", "").strip()
    ...
    def update_segment_images(self, visualization_data):
        ...
        if isinstance(visualization_data, list):
            ...
            self.console_output.append("Warning Received old format list data, will be processed as segment list")
            ...
            self.console_output.append(
                f"Error Expected visualization_data to be a dictionary or list, actual type is {type(visualization_data)}")
            return

        for data in segments:
            if not isinstance(data, dict):
                self.console_output.append(f"Error Expected segment_data to be a dictionary, actual type is {type(data)}")
                continue
            self.segment_list.addItem(f"Action {data['index']}: frame {data['start']} - {data['end']}")
    ...
    def handle_segmentation_result(self, output_files):
        if output_files:
            self.console_output.append(f"Splitting completed, a total of {len(output_files)} files generated")
            self.segmented_files = [f for f in output_files if f.endswith(".npz")]
            self.segmented_files_list.clear()
            for file_path in self.segmented_files:
                self.segmented_files_list.addItem(os.path.basename(file_path))
            self.console_output.append(f"Found {len(self.segmented_files)} .npz files")
            self.recognize_btn.setEnabled(bool(self.segmented_files and self.checkpoint_path))
        else:
            self.console_output.append("No files were generated during the split process")
            self.recognize_btn.setEnabled(False)

    def update_selected_file(self, row):
        if row >= 0 and row < len(self.segmented_files):
            self.selected_segmented_file = self.segmented_files[row]
            self.console_output.append(f"Selected split file: {os.path.basename(self.selected_segmented_file)}")
            self.recognize_btn.setEnabled(bool(self.selected_segmented_file and self.checkpoint_path))
            self.single_recognize_btn.setEnabled(bool(self.selected_segmented_file and self.checkpoint_path))
        else:
            self.selected_segmented_file = None
            self.recognize_btn.setEnabled(False)
