    ...
    def run(self):
        try:
            if self.task_type == "direct_run":
                ...
                from paste import (process_event_data, compute_features, segment_actions,
...
class MainWindow(QWidget):
    ...
    def run_recognition(self):
        ...
        try:
            ...
            # Run classification
            results, summary = self.recognizer.classify_npz_files(npz_file_paths)

            # Update UI with results
            self.result_list.clear()
            for result in results:
                item_text = f"{os.path.basename(result['file'])}: {result['label']} ({result['comm_code']})"
