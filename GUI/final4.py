...
class MainWindow(QWidget):
    def __init__(self):
        ...
        left_layout.addWidget(QLabel("分割文件列表:"))
    ...
    def run_recognition(self):
       ...
       #  self.console_output.append(f"找到 {len(npz_file_paths)} 个 .npz 文件，开始分类...")
        self.console_output.append(f"开始分类选中的文件: {os.path.basename(self.selected_segmented_file)}")
        try:
    ...
    def handle_segmentation_result(self, output_files):
        if output_files:
            ...
            self.console_output.append(f"找到 {len(self.segmented_files)} 个 .npz 文件")
    ...
    def update_selected_file(self, row):
        if row >= 0 and row < len(self.segmented_files):
            ...
            self.console_output.append(f"已选择分割文件: {os.path.basename(self.selected_segmented_file)}")
            # Enable recognize button if checkpoint is selected
