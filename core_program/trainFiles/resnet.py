...
# SpikingResNet implementation (from provided code, included for completeness)
...
def _spiking_resnet(arch, block, layers, pretrained, progress, spiking_neuron, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(state_dict, strict=False)  # Strict=False to ignore mismatched keys
    return model

def spiking_resnet18(pretrained=False, progress=True, spiking_neuron: callable=None, **kwargs):
    return _spiking_resnet('resnet18', BasicBlock, [2, 2, ritel2, 2], pretrained, progress, spiking_neuron, **kwargs)
...
class EventUAVNet(nn.Module):
    def __init__(self, T: int, resnet_variant: str = 'resnet18', use_cupy=False):
        ...
        # Initialize SpikingResNet with LIF neuron and 5 output classes
        self.resnet = resnet_models[resnet_variant](
            pretrained=False,
            progress=True,
            spiking_neuron=neuron.LIFNode,
            num_classes=5,  # UAV dataset has 5 classes
...
def main():
    ...
    parser.add_argument('-opt', type=str, choices=['sgd', 'adam'], default='sgd', help='use which optimizer. SGD or Adam')
    ...
    # Save neuron voltage and spike data
    net.eval()
    output_layer = net.resnet.fc  # Final linear layer
    output_layer.v_seq = []
    output_layer.s_seq = []

    def save_hook(m, x, y):
        output_layer.v_seq.append(x[0].unsqueeze(0))  # Save input to fc layer
    ...
    with torch.no_grad():
        ...
        frame = frame.unsqueeze(1)  # [T, 1, C, H, W]
        out_fr = net(frame)
