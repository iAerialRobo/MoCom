                    ...
                    if os.path.exists(file_path):
                        try:
                            ...
                            if label in range(5):  # Ensure label is valid for 5-class classification
                                ...
                                print(f"Line {line_number}: Invalid label {label} (must be 0-4) for {filename}, skipping.")
                        except ValueError:
                            print(f"Line {line_number}: Invalid label '{label}' (non-integer) for {filename}, skipping.")
                    else:
...
def main():
    ...
    if len(finetune_files) != len(labels):
        print(f"Warning: Mismatch between files ({len(finetune_files)}) and labels ({len(labels)}). Truncating to match.")
    ...
    if minority_files:
        ...
        for epoch in range(10):
            ...
            print(f"Phase 1, Epoch [{epoch+1}/10], Loss: {total_loss/len(minority_dataloader):.4f}, Accuracy: {accuracy:.2f}%, Time: {time.time()-start_time:.2f}s")
    ...
    net.eval()
    # Initialize test dataset with validated files and labels
    labels = global_labels
