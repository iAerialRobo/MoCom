...
class NPZDatasetNegative(DatasetFolder):
    def __init__(self, root, transform=None, target_transform=None):
        super().__init__(root, loader=NPZDatasetNegative.load_npz_frames, extensions=('npz',),
    ...
    def __getitem__(self, index):
        ...
        # Select only negative events (channel 1)
        frames = frames[:, 1:2, :, :]  # Shape: (T, 1, H, W)
        frames = torch.tensor(frames, dtype=torch.float32)
        frames = self.resize_frames(frames)
        indices = torch.linspace(0, frames.shape[0] - 1, steps=8).long()
...
class EventUAVNet(nn.Module):
    def __init__(self, channels=128, spiking_neuron: callable = None, **kwargs):
        ...
        for i in range(2):
            if conv.__len__() == 0:
                in_channels = 1  # Changed to 1 for negative events only
            else:
...
def main():
    parser = argparse.ArgumentParser(description='Classify UAV Events (Negative Only)')
    ...
    parser.add_argument('-data-dir', type=str, help='root dir of UAV event dataset')
    parser.add_argument('-out-dir', type=str, default='./logs_negative', help='root dir for saving logs and checkpoint')
   ...
   #  args.data_dir = '/home/zhangnb/Tro/data/data_frame/'
    ...
    train_data_loader = DataLoader(
        dataset=train_set, batch_size=args.b, shuffle=True, drop_last=True,
        num_workers=args.j, pin_memory=True
    )
    test_data_loader = DataLoader(
        dataset=test_set, batch_size=args.b, shuffle=True, drop_last=False,
        num_workers=args.j, pin_memory=True
    )
    ...
    net.to(args.device)
    out_dir = os.path.join(args.out_dir, f'T{args.T}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}_negative')
    if args.amp:
        out_dir += '_amp'
    if args.cupy:
        out_dir += '_cupy'
