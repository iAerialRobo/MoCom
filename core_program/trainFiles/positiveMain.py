...
class NPZDatasetPositive(DatasetFolder):
    def __init__(self, root, transform=None, target_transform=None):
        super().__init__(root, loader=NPZDatasetPositive.load_npz_frames, extensions=('npz',),
    ...
    def __getitem__(self, index):
        ...
        # Select only positive events (channel 0)
        frames = frames[:, 0:1, :, :]  # Shape: (T, 1, H, W)
...
class EventUAVNet(nn.Module):
    def __init__(self, channels=128, spiking_neuron: callable = None, **kwargs):
        ...
        for i in range(2):
            if conv.__len__() == 0:
                in_channels = 1  # Changed to 1 for positive events only
            else:
...
def main():
    parser = argparse.ArgumentParser(description='Classify UAV Events (Positive Only)')
    ...
    parser.add_argument('-out-dir', type=str, default='./logs_positive', help='root dir for saving logs and checkpoint')
    ...
    full_dataset = NPZDatasetPositive(root=args.data_dir)
    ...
    net.to(args.device)
    out_dir = os.path.join(args.out_dir, f'T{args.T}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}_positive')
    if args.amp:
        out_dir += '_amp'
    if args.cupy:
        out_dir += '_cupy'
